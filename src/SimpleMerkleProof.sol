...
contract SimpleMerkleProof {
	...
	function lookUp(bytes32 root, bytes memory key,  Item[] memory db) internal view returns (bytes memory v) {
		bytes32 hash = root;
				...
				if (kind == NODEKIND_NOEXT_LEAF) {
					...
					if (startsWith(partialKey, branch.key)) {
						if (partialKey.length == sliceLen) {
							...
							Node.NodeHandle memory child = branch.children[index];
							if (child.exist) {
	...
	function getNodeData(bytes32 hash, Item[] memory db) internal view returns (bytes memory) {
